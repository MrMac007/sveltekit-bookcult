 ...
 */
export async function getOrFetchBook(googleBooksId: string): Promise<Book | null> {
  ...
  try {
    ...
    if (upsertedBook) {
      ...
      console.warn('[book-cache] Upsert returned null, falling back to normalized book with Google Books ID:', book.id)
      return book
    }
  } catch (error) {
